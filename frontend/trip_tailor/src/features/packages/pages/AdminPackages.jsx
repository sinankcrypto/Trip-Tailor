...
const AdminPackagesPage = () => {
  ...
  const { packages, pagination, loading, error } = useGetAllPackages(filters);
            ...
            {packages.length === 0 ? (
              ...
              packages.map((pkg) => (
                <tr key={pkg.id} className="hover:bg-gray-50">
                  ...
                  <td className="border px-4 py-2 font-medium text-gray-800">{pkg.title}</td>
                  <td className="border px-4 py-2 text-gray-700">{pkg.agency_name}</td>
                  <td className="border px-4 py-2 text-gray-700">₹{pkg.price}</td>
                  <td className="border px-4 py-2 text-gray-700">{pkg.duration} days</td>
